# d-Con %QAAUTOMATION_SCRIPTS%\GameStopRespMobile\spec\UI\mgs_omniture_burger_links_spec.rb  --csv %QAAUTOMATION_SCRIPTS%\GameStopRespMobile\spec\UI\mgs_responsive_dataset.csv --range header003 --browser chrome --or
...
describe "MGS NextGen Omniture Burger Links Behavior" do
  ...
  after(:each) do
	...
	after(:all) do
		...
		if $has_run == false
			warn("
			\r\n***********************************************************
      \r\n  VALIDATING OMNITURE VALUES FOR PHONES HAVE BEEN SKIPPED.
	...
	it "Mobile Omniture burger link validation for phones" do
		if @device.upcase.strip == "PHONE"
			$has_run = true
			$proxy.start_capture
			@browser.open("#{@start_page}")

			$tracer.report("===== >>>>> Navigate burger links.")
			@browser.mgs_hdr_mobile_hamburger.click
			sleep 2
			if @params['login'] == ""
				@browser.mgs_hdr_mobmnu_signin_lnk.click if @browser.mgs_hdr_mobmnu_signin_lnk.exists == true
				sleep 4
				@browser.url_data.full_url.to_s.include?("loginqa.gamestop.com/Account/Login?ReturnUrl=http%3A%2F%2Fqa.m2.gamestop.com%2F")
				signin_page = @browser.url_data.full_url
        puts "===== >>>>> #{signin_page}"
			else
				@browser.mgs_hdr_mob_myaccount_lnk.click if @browser.mgs_hdr_mob_myaccount_lnk.exists == true
        sleep 4
				@browser.url_data.full_url.to_s.include?("loginqa.gamestop.com/Profile")
        myaccount_page = @browser.url_data.full_url
			end
			@browser.browser_back

			@browser.mgs_hdr_mobmnu_shop_lnk.click if @browser.mgs_hdr_mobmnu_shop_lnk.exists == true
			sleep 3
			@browser.url_data.full_url.should == "#{@start_page}/browse"
			shop_page = @browser.url_data.full_url
			@browser.browser_back

			@browser.mgs_hdr_mobmnu_deals_lnk.click if @browser.mgs_hdr_mobmnu_deals_lnk.exists == true
			sleep 3
			@browser.url_data.full_url.should == "#{@start_page}/browse?navState=/_/N-9u"
			deals_page = @browser.url_data.full_url
			@browser.browser_back

			@browser.mgs_hdr_mobmnu_trade_lnk.click if @browser.mgs_hdr_mobmnu_trade_lnk.exists == true
      sleep 2
			@browser.url_data.full_url.to_s.include?("qa.gamestop.com/trade")
			trade_page = @browser.url_data.full_url
			@browser.browser_back

			@browser.mgs_hdr_mobmnu_pur_lnk.click if @browser.mgs_hdr_mobmnu_pur_lnk.exists == true
			sleep 2
			@browser.url_data.full_url.to_s.include?("m.qa.gamestop.com/poweruprewards")
			pur_page = @browser.url_data.full_url
			@browser.browser_back

			@browser.mgs_hdr_mobmnu_orders_lnk.click if @browser.mgs_hdr_mobmnu_orders_lnk.exists == true
			sleep 2
			@browser.url_data.full_url.to_s.include?("qa.m2.gamestop.com/Orders/OrderHistoryLogin")
			orders_page = @browser.url_data.full_url
			@browser.browser_back

			@browser.mgs_hdr_mobmnu_gc_lnk.click if @browser.mgs_hdr_mobmnu_gc_lnk.exists == true
			sleep 3
			@browser.url_data.full_url.to_s.include?("qa.m2.gamestop.com/gift-cards")
			gift_page = @browser.url_data.full_url
			@browser.browser_back

			if @params['login'] == ""
				#@browser.mgs_hdr_mobmnu_list[8].a.click if @browser.mgs_hdr_mobmnu_list[8].a.exists == true
        @browser.li.id('mobile-createAccount').a.click
				sleep 2
				...
				newaccount_page = @browser.url_data.full_url
			else
				@browser.mgs_hdr_mob_signout_lnk.click if @browser.mgs_hdr_mob_signout_lnk.exists == true
				sleep 2
				@browser.url_data.full_url.to_s.include?("qa.m2.gamestop.com//Frame/Header/MobileSignOut")
				signout_page = @browser.url_data.full_url
			end
			@browser.browser_back
			...
			if @params['login'] == ""
				query_string_value  = get_value_from_query_strings(omniture_list, signin_page, query_string_name)
				query_string_value.should == "mobilenav|sign in"
				$tracer.report("Query String Value = #{query_string_value}")
      else
				query_string_value  = get_value_from_query_strings(omniture_list, myaccount_page, query_string_name)
				query_string_value.should == "mobilenav|my account"
				$tracer.report("Query String Value = #{query_string_value}")
      end

			query_string_value  = get_value_from_query_strings(omniture_list, shop_page, query_string_name)
			query_string_value.should == "mobilenav|shop"
			$tracer.report("Query String Value = #{query_string_value}")

			query_string_value  = get_value_from_query_strings(omniture_list, deals_page, query_string_name)
			query_string_value.should == "mobilenav|deals"
			$tracer.report("Query String Value = #{query_string_value}")

			query_string_value  = get_value_from_query_strings(omniture_list, trade_page, query_string_name)
			query_string_value.should == "mobilenav|trade"
			$tracer.report("Query String Value = #{query_string_value}")


      # pur is not monitored and skipped here
			query_string_value  = get_value_from_query_strings(omniture_list, gift_page, query_string_name)
			query_string_value.should == "mobilenav|gift cards"
			$tracer.report("Query String Value = #{query_string_value}")

			query_string_value  = get_value_from_query_strings(omniture_list, orders_page, query_string_name)
			query_string_value.should == "mobilenav|guest / in store orders"
			$tracer.report("Query String Value = #{query_string_value}")

			if @params['login'] == ""
				query_string_value  = get_value_from_query_strings(omniture_list, newaccount_page, query_string_name)
				query_string_value.should == "mobilenav|create an account"
				$tracer.report("Query String Value = #{query_string_value}")
      else
				query_string_value  = get_value_from_query_strings(omniture_list, signout_page, query_string_name)
				query_string_value.should == "mobilenav|sign out"
				$tracer.report("Query String Value = #{query_string_value}")
			end
		else
			$tracer.trace("
