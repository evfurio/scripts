...
# d-Con %QAAUTOMATION_SCRIPTS%\GameStopRespMobile\spec\UI\mgs_cart_display_spec.rb  --csv %QAAUTOMATION_SCRIPTS%\GameStopRespMobile\spec\UI\mgs_responsive_dataset.csv --range TFS19319A --browser chrome --or
# d-Con %QAAUTOMATION_SCRIPTS%\GameStopRespMobile\spec\UI\mgs_cart_display_spec.rb  --csv %QAAUTOMATION_SCRIPTS%\GameStopRespMobile\spec\UI\mgs_responsive_dataset.csv --range TFS19319B --browser chrome --or
...
describe "MGS NextGen Footer Fullsite Links Behavior" do
  ...
  it "Mobile fullsite link validation for tablet" do
    if @device.upcase.strip == "TABLET"
      ...
      @myText.each_index do |filter|
        @browser.mgs_search_filter_list[0].ul[0].li[filter].a.click if @myText[filter].to_s.include?("Usually ships in 24 hours")
      end
      sleep 2
  ...
  it "Mobile fullsite link validation for phones" do
    if @device.upcase.strip == "PHONE"
      ...
      @browser.div.className("empty_cart").h4.should_not_exist == true
		@browser.name_link().is_visible.should.eql?(true)
		@browser.name_link().to_s.include?("#{@product_search_key}")
