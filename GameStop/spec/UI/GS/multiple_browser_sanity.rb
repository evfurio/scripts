...
# d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\UI\GS\multiple_browser_sanity.rb  --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\UI\GS\checkout_dataset.csv --range TFS48627 --login yourprod@account.com --password yourpassword --sql %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\sql_queries\get_single_product.sql --db %QAAUTOMATION_SCRIPTS%\GameStop\Spec\databases.csv --db_env "QA_Catalog" --prop %QAAUTOMATION_SCRIPTS%\GameStop\Spec\properties.csv --url PROD_GS --or
...
$arr = ["chrome", "firefox"] #, "ie-webspec", "safari-webspec", "safari", "ie"]
...
$arr.each { |b|
  describe "Sanity for All Browsers" do
    ...
    it "#{$tc_id} should login on gs.com and refresh page with #{b}" do
    ...
    it "#{$tc_id} should login from checkout and refresh page with #{b}" do
      $browser.cookie.all.delete
      $browser.instance_variable_get(:@driver).navigate.refresh
      $browser.open("#{@start_page}/checkout")
      $browser.log_in_link.click
