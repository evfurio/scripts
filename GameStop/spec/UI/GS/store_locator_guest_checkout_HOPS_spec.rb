##USAGE NOTES
##
##d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\UI\GS\store_locator_guest_checkout_HOPS_spec.rb  --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\UI\GS\checkout_dataset.csv --range TFS12345  --browser chrome  --or
...
describe "Store Locator with HOPS" do
  before(:all) do
  ...
  it "#{$tc_id} #{$tc_desc}" do
    @browser.open("#{@start_page}")
    @browser.start_find_store(@params["bill_zip"])
    ...
    if @params["unsubscribe"]
      # @browser.hops_zip_code_search_field.value = 'ohio'
      # @browser.hops_zip_code_search_button.click
      # sleep 2
      @browser.hops_store_list.at(0).pickup_at_store_button.click
    end

    @browser.pickup_at_store_address.should_exist
    $tracer.trace(@browser.pickup_at_store_address.innerText)
    @browser.pickup_at_store_hours.should_exist
    $tracer.trace(@browser.pickup_at_store_hours.innerText)
