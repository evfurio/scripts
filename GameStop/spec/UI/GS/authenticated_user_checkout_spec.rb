...
describe "Authenticated User Checkout" do
  ...
  it "#{$tc_id} #{$tc_desc}" do
    ...
    if @params["do_linkshare"] #|| @params["do_ef"]
      ...
      #TODO - Validation methods for affiliate cookies, URL expectation and AffiliateHit DB database
    else
    ...
    @browser.validate_cart_badge(@session_id, @cart_id, @cart_svc, @cart_svc_version, @params) unless @params["do_merge_cart"]
...
#TODO : Get shipping address from profile service and validate the address is populated
      ...
      @browser.paypal_payment_selector.should_exist unless @params["do_ispu"]
    ...
    if @params["submit_order_flag"]
      ...
      if @params["load_ship_addr_from_profile"]
        ...
        if !@params["use_paypal_at_cart"] && !@params["use_paypal_at_payment"]
          @browser.check_shipping_address_equivalence_against_params(@shipment.ship_to, @params) if @shipment != nil
        end
      end
    end
  end
