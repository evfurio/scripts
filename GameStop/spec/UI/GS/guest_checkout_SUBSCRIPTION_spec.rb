# d-Con  %QAAUTOMATION_SCRIPTS%\GameStop\Spec\UI\GS\guest_checkout_SUBSCRIPTION_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\UI\GS\checkout_dataset.csv --range TFS57595 --browser chrome --or 
...
describe "Guest Checkout DigiMag Subscription 12 Month" do
  before(:all) do
    ...
    @browser.add_powerup_rewards_number(@params)
      ...
      @browser.enter_handling_options(@params) if @physical_product				
        ...
        @browser.validate_cookies_and_tags(@params)
				@browser.validate_cart_badge(@session_id, @cart_id = nil, @cart_svc, @cart_svc_version, @params)
        @browser.check_checkout_billing_fields_exist
        @browser.continue_checkout_button.click
			end
			@browser.continue_checkout_button.click
