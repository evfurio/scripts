### *** Recommerce *** 
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\spec\UI\RCM\rcm_validate_poor_recycled_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\spec\UI\RCM\tradevalue_dataset.csv --range TFS40147 --browser chrome --or 
...
describe "Trade Value - Search for device" do
	...
	it "should be able to search for device" do
		...
		@browser.open(url+"/ProductCondition.aspx?producttitle=Apple+iPhone+4+8GB+AT%26T+-+A1332")
		@browser.wait_for_landing_page_load
		...
		unless @browser.credit_price_label.innerText == "Recycle"
			credit_price_poor = money_string_to_decimal(@browser.credit_price_label.inner_text)
		end
		unless @browser.cash_price_label.innerText == "Recycle"
			cash_price_poor = money_string_to_decimal(@browser.cash_price_label.innerText)
		end
		
		
		puts "============================ credit_price_poor :::::    #{credit_price_poor}"
		puts "============================ cash_price_poor :::::    #{cash_price_poor}"
		...
		sleep 5
		puts "============================ credit_price_poor_after :::::    #{@browser.credit_price_label.inner_text}"
		puts "============================ cash_price_poor_after :::::    #{@browser.cash_price_label.innerText}"
		
		
		@recycle_credit = (@browser.credit_price_label.inner_text.eql?("Recycle") ? true : false )
		credit_price_poor_after = (@recycle_credit ? 0 : money_string_to_decimal(@browser.credit_price_label.inner_text))
		
		@recycle_cash = (@browser.cash_price_label.inner_text.eql?("Recycle") ? true : false )
		cash_price_poor_after = (@recycle_cash ? 0 : money_string_to_decimal(@browser.cash_price_label.inner_text))
		
		puts "============================ credit_price_poor_after :::::    #{credit_price_poor_after}"
		puts "============================ cash_price_poor_after :::::    #{cash_price_poor_after}"
		
		
		#ASSERT: Cash Price / Credit Price should not be equal after clicking the checkbox.
		@expected_result = true
		actual_credit = (credit_price_poor != credit_price_poor_after ? true : false)
		actual_cash = (cash_price_poor != cash_price_poor_after	? true : false)
