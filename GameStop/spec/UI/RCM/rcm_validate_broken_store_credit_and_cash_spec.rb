### *** Recommerce *** 
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\spec\UI\RCM\rcm_validate_broken_store_credit_and_cash_spec.rb --prop %QAAUTOMATION_SCRIPTS%\Common\config\properties.csv  --url QA_GS --csv %QAAUTOMATION_SCRIPTS%\GameStop\spec\UI\RCM\tradevalue_dataset.csv --range TFS40146 --browser chrome --or 
...
describe "Trade Value - Search for device" do
	...
	it "should be able to search for device" do
		...
		@browser.open(url+"/ProductCondition.aspx?producttitle=Apple+iPad+4+WiFi+128GB+-+A1458")
		@browser.wait_for_landing_page_load
		...
		puts "---------------------------------------------------------  #{@browser.rcm_tab1_section.call("style.display")}"
		...
		credit_price_broken = money_string_to_decimal(@browser.credit_price_label.inner_text)
		cash_price_broken = money_string_to_decimal(@browser.cash_price_label.innerText)
		puts "============================ credit_price_broken :::::    #{credit_price_broken}"
		puts "============================ cash_price_broken :::::    #{cash_price_broken}"
		...
		sleep 5
		puts "============================ credit_price_broken_after :::::    #{@browser.credit_price_label.inner_text}"
		puts "============================ cash_price_broken_after :::::    #{@browser.cash_price_label.innerText}"
		
		
		@recycle_credit = (@browser.credit_price_label.inner_text.eql?("Recycle") ? true : false )
		credit_price_broken_after = (@recycle_credit ? 0 : money_string_to_decimal(@browser.credit_price_label.inner_text))
		
		@recycle_cash = (@browser.cash_price_label.inner_text.eql?("Recycle") ? true : false )
		cash_price_broken_after = (@recycle_cash ? 0 : money_string_to_decimal(@browser.cash_price_label.inner_text))
		
		puts "============================ credit_price_broken_after :::::    #{credit_price_broken_after}"
		puts "============================ cash_price_broken_after :::::    #{cash_price_broken_after}"
		
		
		#ASSERT: Cash Price / Credit Price should not be equal after clicking the checkbox.
		@expected_result = true
		actual_credit = (credit_price_broken != credit_price_broken_after ? true : false)
		actual_cash = (cash_price_broken != cash_price_broken_after	? true : false)
