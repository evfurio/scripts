...
#TODO - if the parameters are passed and the user is not registered, register the new user and authorize
    ...
    @open_id = @account_svc.perform_authorization_and_return_user_and_open_id(@session_id, user_name, user_password, @account_svc_version)
    @profile_svc.perform_get_addresses(@open_id, @session_id, @client_channel, @profile_svc_version)
    @owner_id = @profile_svc.perform_get_cart_id(@open_id, @session_id, @client_channel, @profile_svc_version)

    #Conditional statement to report Guest or Authenticated User ID.
    if user_name == nil
      $tracer.report("Owner Id for Guest User :: #{@owner_id}")
    else
      $tracer.report("User: #{user_name}'s Owner Id :: #{@owner_id}")
      cart_clean = @cart_svc.perform_empty_cart(@session_id, @owner_id, @client_channel, @locale, @cart_svc_version)
    end
    $tracer.report(" User : #{user_name}\n Password : #{user_password}\n OpenId : #{@open_id}\n Owner Id : #{@owner_id}\n Cart Cleared? : #{cart_clean}")
  end
  ...
  it "should Checkout using dataset defined" do
      ...
      if @complete_checkout
        #(username, password, session_id, owner_id, client_channel, locale, row, cc, version)
        purchase_rsp = @purchase_order_svc.perform_purchase_with_generated_credit_card(@session_id, @owner_id, @client_channel, @locale, @row, @purchase_order_svc_version)
