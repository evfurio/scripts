...
#get results from the sql file
		...
		#@owner_id = "f141001c-29eb-401c-b84f-778c2f242bf1"
		@owner_id = "2cf4b873-3f50-4bb8-8b96-df719f0243b0"
		...
		@profile_svc.get_address_by_user_id(@owner_id, @session_id, @client_channel, @profile_svc_version)
...
#Add products to cart
	...
	def initialize_payment_params
		...
		#adding logic for getting generated credit cards
		# begin
			# @credit_card = @row.find_value_by_name("CreditCard")
				# if @credit_card = "Generate"
					# raise
				# end
			# @cc_type = @row.find_value_by_name("CCType")
		# rescue Exception => ex
			# @cc_type_flag = @row.find_value_by_name("CCType")
			# @cvv = @row.find_value_by_name("CVV")
		
			# if @cc_type_flag.capitalize == "VISA"
				# @cc_type = "Visa"
				# @credit_card = CreditCard.visa
				# $tracer.trace("VISA Number : #{@credit_card}")
			# end	
		
			# if @cc_type_flag.capitalize == "MASTERCARD"
				# @cc_type = "MasterCard"
				# @credit_card = CreditCard.mastercard
				# $tracer.trace("MasterCard Number : #{@credit_card}")
			# end
			
			# if @cc_type_flag.capitalize == "AMEX"
				# @cc_type = "Amex"
				# @credit_card = CreditCard.amex
				# $tracer.trace("Amex Number : #{@credit_card}")
			# end
			
			# if @cc_type_flag.capitalize == "DISCOVER"
				# @cc_type = "Discover"
				# @credit_card = CreditCard.discover
				# $tracer.trace("Discover Number : #{@credit_card}")
			# end
