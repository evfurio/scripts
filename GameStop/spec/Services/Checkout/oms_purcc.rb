# d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_purcc.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_dataset.csv --range "OMS01"  --or
...
describe "OMS Web Order Scenarios" do
  ...
  it "#{$tc_id} #{$tc_desc}" do
      ...
      $tracer.trace("Sku/Qty :::::   #{Hash[*sku_qty_list]}")
      ...
      if @params['Checkout']
        ...
        if @params['pay_type'].downcase == "cc"
          ...
          purchase_rsp = @purchase_order_svc.perform_purchase_with_credit_card(@session_id, @owner_id, @params["ClientChannel"], @params["Locale"], @params, @purchase_order_svc_version)
					...
					# $tracer.report("Service Response: #{@auth_settle_rsp.http_body.find_tag("result").content}       Expected: Approved")
					# @auth_settle_rsp.http_body.find_tag("result").content.should == "Approved"
