...
#[TestMethod]
    it "should Checkout using dataset defined" do
		...
		#this is to set the number of orders to be ran.  please do not check this in with more than 1 run.  
		#the purpose of iterative tests is for order pumping or redundancy checks.  MTM should only run this test once.
		iteration = 0
		count = 0
		orders = 100
		...
		puts "THIS IS THE USER NAME"
		puts @user_name
		
		auth_rsp = @account_svc.perform_authorization_and_return_user_and_open_id(@session_id, @user_name, @user_password, @account_svc_version)
		@openid = auth_rsp[0]
		@userid = auth_rsp[1]
		
		#@profile_svc.get_address_by_user_id(@userid, @session_id, @client_channel, @profile_svc_version)
		#profile_rsp = @profile_svc.get_user_profile(@userid, @session_id, @client_channel, @profile_svc_version)
		#puts profile_rsp
        ...
        @purchase_order_svc.perform_add_shipping_addresses_to_shipments_with_user(@session_id, @owner_id, @client_channel, @locale,  @purchase_order_svc_version, @row, purchase_order_shipment_ids, @user_name)
		
		### ADD_CUSTOMER_TO_PURCHASE_ORDER ###
		puts "THIS IS THE USER NAME"
		puts @user_name
		@purchase_order_svc.perform_add_new_customer_to_purchase_order(@session_id, @owner_id, @client_channel, @locale,  @purchase_order_svc_version, @row, @user_name)
		...
		if @complete_checkout == "true"
			#Generate the Credit Card Information
			card_type = @row.find_value_by_name("CCType")
	    	@cc = @purchase_order_svc.generate_credit_card(card_type)
			####
			
			purchase_rsp = @purchase_order_svc.perform_purchase_with_generated_credit_card(@user_name, @user_password, @session_id, @owner_id, @client_channel, @locale, @row, @cc, @purchase_order_svc_version)
			...
			### SAVE PAYMENT TO DIGITAL WALLET ###
			@payment_svc.perform_save_generated_payment_methods_to_wallet(@client_channel, @name_on_card, @openid, @cc, @payment_svc_version)
	...
	def initialize_billing_addr_params
		...
		@name_on_card = "#{@bill_first_name} #{@bill_last_name}"
	end
