# d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_perf_cc_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_dataset.csv --range "OMS01"  --or
...
describe "Creditcard OMS PerfTest" do
	...
	def results_to_csv(tc_id, order_number, time)
		...
		if dump_to_csv == "true"
			...
			test_info.each do |i|
				...
				case tc_id
					when 'OMS01'
						CSV.open("#{path}\\oms_cc_mc_results.csv", "a") do |csv|
							csv << @data
						end
					when 'OMS02'
						CSV.open("#{path}\\oms_cc_visa_results.csv", "a") do |csv|
							csv << @data
						end
					when 'OMS03'
						CSV.open("#{path}\\oms_cc_discover_results.csv", "a") do |csv|
							csv << @data
						end
					when 'OMS04'
						CSV.open("#{path}\\oms_cc_amex_results.csv", "a") do |csv|
							csv << @data
						end	
				end
			end
