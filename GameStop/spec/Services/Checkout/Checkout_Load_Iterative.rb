...
###
####################################################################################################################################################################################################################################
###                                        Single execution in DEV using available products (not digital or download),                                                                                                           ###          
#################################################################################################################################################################################################################################### 
###
### Single execution in DEV using an available product (not digital or download), **Guest User, Checkout, 
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_template.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_dataset.csv --range "TFS44313" --sql %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\sql_queries\get_products.sql --db %QAAUTOMATION_SCRIPTS%\GameStop\Spec\databases.csv --db_env "DEV_Catalog" --svc %QAAUTOMATION_SCRIPTS%\GameStop\Spec\endpoints.csv --svc_env DEV_V1 --or
###
####################################################################################################################################################################################################################################
###                                        Single execution in QA2 using available products (not digital or download),                                                                                                           ###          
#################################################################################################################################################################################################################################### 
###
##$ **Guest User, Checkout ALL RUN TIME PARAMETERS IN THE COMMAND LINE
...
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_template.rb --login auto_generic_qa@gamestop.com --password T3sting --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_dataset.csv --range "TFS44309" --sql %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\sql_queries\get_products.sql --db %QAAUTOMATION_SCRIPTS%\GameStop\Spec\databases.csv --db_env "QA_Catalog" --svc %QAAUTOMATION_SCRIPTS%\GameStop\Spec\endpoints.csv --svc_env QA2_V1 --or
###
###  **Guest User, Checkout ONLY REQUIRED PARAMETERS IN THE COMMAND LINE
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_template.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_dataset.csv --range "TFS44309"  --or
###
###  **Authenticated User, Checkout ONLY REQUIRED PARAMETERS IN THE COMMAND LINE
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_template.rb --login auto_generic_qa@gamestop.com --password T3sting --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_dataset.csv --range "TFS44309"  --or
####################################################################################################################################################################################################################################
#**HACK for Load Testing**# QA2_Hack
#d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\Checkout_Load_Iterative.rb --login auto_generic_qa@gamestop.com --password T3sting --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\checkout_dataset.csv --range "TFS44309" --sql %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\sql_queries\get_single_product.sql --db %QAAUTOMATION_SCRIPTS%\GameStop\Spec\databases.csv --db_env "QA_Catalog" --svc %QAAUTOMATION_SCRIPTS%\GameStop\Spec\endpoints.csv --svc_env QA2_V2 --or
...
#We will initialize our connections and data sets for the test by taking the option parameters passed by the command line 
    ...
    before(:each) do	
		...
		@cart_svc = @global_functions.cart_svc
		@account_svc = @global_functions.account_svc
		@purchase_order_svc = @global_functions.purchaseorder_svc
		@shipping_svc = @global_functions.shipping_svc
		@icheckout_svc = @global_functions.checkout_svc
		@tax_svc = @global_functions.tax_svc
...
#passes parameters to the DSL to get the ownerid.  the ownerid is used throughout the remainder of the script as the identifier for the cart/purchaseorder
        ...
        @cart_svc.perform_empty_cart(@session_id, @owner_id, @client_channel, @locale)
    end
	iterations = 5
	ac = 0	
...
#Add products to cart
		...
		if create_purchase_order_rsp.http_body.find_tag("shipment").size > 1
		   $tracer.trace("IMPORTANT::::: There is a problem in d-Con with addition child elemnt to second shipment item. This script fails if there are 2 shipment groups. You will get 'index out of range, referencing element 1' ")
		end
		...
		create_purchase_order_rsp.http_body.find_tag("shipment").each_with_index do |po_shipment, i|
			...
			puts add_shipping_methods_rsp.http_body.formatted_xml
		end	
...
##getting this from the database but haven't set this up for multiple products as of yet
			...
			tax_shipment_data.purchase_date.remove_self
		...
		if @complete_checkout == "true"
			...
			purchase_req = @purchase_order_svc.get_request_from_template_using_global_defaults("purchase", PurchaseOrderServiceRequestTemplates::PURCHASE_WITH_CREDIT_CARD)
...
#####################################################################################################################			
			...
			puts " TAX::::::::::::::::: #{total_tax}"
			purchase_rsp.http_body.find_tag("total_tax").content.should == total_tax
			...
			expected_total = (expected_subtotal.to_f + calculated_shipping_cost.to_f).round(2)  + total_tax.to_f 
			actual_total.to_s.should == expected_total.to_s
		...
		#Remember split orders will append a 'D' or 'P' for the split
			...
			if purchase_rsp.http_body.find_tag("purchase_successful").content == "true"
				get_purchase_order_by_tracking_number_req = @purchase_order_svc.get_request_from_template_using_global_defaults("get_purchase_order_by_tracking_number", PurchaseOrderServiceRequestTemplates::GET_PURCHASE_ORDER_BY_TRACKING_NUMBER)
