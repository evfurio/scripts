# d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_checkout_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_dataset.csv --range "TFS18125"  --or
	...
	def results_to_csv(tc_id, order_number, time)
		...
		if dump_to_csv == "true"
			...
			test_info.each do |i|
				...
				CSV.open("#{path}\\oms_service_regression_results.csv", "a") do |csv|
					csv << @data
				end
			end
		end
