# d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_perf_purcc_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Checkout\oms_dataset.csv --range "OMS09"  --or
...
describe "PURCC OMS PerfTest" do
	...
	def results_to_csv(tc_id, order_number, time)
		...
		if dump_to_csv == "true"
			...
			test_info.each do |i|
				...
				case tc_id
					when 'OMS09'
						CSV.open("#{path}\\oms_purcc_4241_results.csv", "a") do |csv|
							csv << @data
						end
					when 'OMS10'
						CSV.open("#{path}\\oms_purcc_4282_results.csv", "a") do |csv|
							csv << @data
						end
					when 'OMS11'
						CSV.open("#{path}\\oms_purcc_4324_results.csv", "a") do |csv|
							csv << @data
						end
					when 'OMS12'
						CSV.open("#{path}\\oms_purcc_4365_results.csv", "a") do |csv|
							csv << @data
						end	
				end
			end
