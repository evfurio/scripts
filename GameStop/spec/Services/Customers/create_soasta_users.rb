...
###USAGE NOTES
###
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Customers\create_soasta_users.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Customers\create_users_and_wallets.csv --range TFSTEST --svc %QAAUTOMATION_SCRIPTS%\GameStop\Spec\endpoints.csv --svc_env QA_V1 --or
###
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Customers\create_soasta_users.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Services\Customers\create_users_and_wallets.csv --range TFSTEST --svc %QAAUTOMATION_SCRIPTS%\GameStop\Spec\endpoints.csv --svc_env PROD_V1 --or
###########################################################################################################################################################################################################
...
#[TestMethod]
    it "Should Create a New SOASTA production User and Populate the Digital Wallet" do
	count = 0
	users = 1000
	i = 26	
	target = "Soasta"
	...
	while count < users do
		if i < 100
			user = "Gamestop.#{target}0#{i}@gmail.com"
			password = "Gamestop0#{i}"
		end
		if i >= 100
			user = "Gamestop.#{target}#{i}@gmail.com"
			password = "Gamestop#{i}"
		end
		puts "#{user} / #{password} \n"
...
#########SAVE_PAYMENT_METHODS TO WALLET###################
		...
		test_info << "#{@username},#{@password},#{@cc_number = nil}.#{@cc_exp_month = nil},#{@cc_exp_year = nil},#{cctype = nil},#{@user_id}"
		...
		# save_payment_methods_to_wallet_req = @payment_svc.get_request_from_template_using_global_defaults(:save_payment_methods_to_wallet,PaymentServiceRequestTemplates.const_get("SAVE_PAYMENT_METHODS_TO_WALLET#{@payment_svc_version}"))
			  ...
			  # save_payment_methods_to_wallet_data.stored_value_card.remove_self
			  # save_payment_methods_to_wallet_data.electronic_account.remove_self
