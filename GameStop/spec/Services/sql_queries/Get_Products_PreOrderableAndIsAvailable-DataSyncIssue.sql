SELECT c.releasedate, c.availability, c.isavailable, d.searchable, D.variantid AS SKU,D.ProductID AS PRODUCTID, i.skuvariantid, m.[variant.SKU], A.DisplayName AS DISPLAY_NAME, D.cy_list_price AS LIST_PRICE, i.OnHandQuantity, m.[variant.Availability], D.InternationalShippingRestrictions as ShipRestrictions
...
(SELECT y.ProductID , count(*) 'Variant Count' 
	...
	--WHERE c.variantid = '831313'
	WHERE c.availability = 'PR' and i.onhandquantity >= 0 and d.searchable = 1
	--WHERE c.availability = 'A' AND c.isavailable = 1 AND c.variantid <> '0' AND c.variantid <> '' 
	--	AND d.searchable =1 AND D.InternationalShippingRestrictions is not Null and D.InternationalShippingRestrictions != 'No Restriction'