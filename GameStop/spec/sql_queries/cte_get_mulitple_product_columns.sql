...
--SELECT * FROM CTE_GameStop
-- Compbines 3 separate query results in separate columns
-- SKU1 = Available New or Used products, SKU2 = Digital, SKU3 = Pre-Order
SELECT top 10 Refurb
FROM
--(select Distinct top 10 VariantId AS New, ListPrice
--from CTE_GameStop where condition = 'New') As New,
--(select  top 10 VariantId AS Used, ListPrice
--from CTE_GameStop where condition = 'Used') AS Used,
(select Distinct top 10 VariantId AS Refurb, ListPrice
from CTE_GameStop where condition = 'Refurbished') AS Refurb
