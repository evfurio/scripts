...
### d-Con %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Sandbox\create_users_populate_wallet.rb --csv %QAAUTOMATION_SCRIPTS%\GameStop\Spec\Sandbox\create_users_and_wallets.csv --range TFSTEST --svc %QAAUTOMATION_SCRIPTS%\GameStop\Spec\endpoints.csv --svc_env QA_V1 --or
###
#############################################################################################################################################################################################################################################################################
###
...
#[TestMethod]
	...
	#generate a new session id guid.  the generate_guid function uses uuidtools to generate guids on the fly.
		...
		creds = @account_svc.create_new_user_return_credentials(@owner_id, @session_id, @user_name, @user_password, @account_svc_version)
...
#########SAVE_PAYMENT_METHODS TO WALLET###################
		...
		save_payment_methods_to_wallet_req = @payment_svc.get_request_from_template_using_global_defaults(:save_payment_methods_to_wallet,PaymentServiceRequestTemplates.const_get("SAVE_PAYMENT_METHODS_TO_WALLET#{@payment_svc_version}"))
			  ...
			  credit_card_payment = save_payment_methods_to_wallet_data.payment_methods.payment_method.at(0)
