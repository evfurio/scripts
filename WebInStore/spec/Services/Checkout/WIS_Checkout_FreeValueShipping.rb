#################################################################################################################################
###This script is used to checkout single or multiple available products by applying free value shipping discount - no PUR applied
#################################################################################################################################
###USAGE NOTES
### Run this script using the following command for single or multi cart checkout
### d-Con %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\Services\Checkout\WIS_Checkout_FreeValueShipping.rb --csv %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\Services\Checkout\wis_checkout_dataset.csv --range "TFS46300"  --or
...
##getting this from the database but haven't set this up for multiple products as of yet
		...
		puts calculate_tax_rsp.http_body.formatted_xml
		...
		if @complete_checkout == "true"
			...
			actual_subtotal.to_s.should == round(expected_subtotal)
			...
			round(actual_total).should == round(expected_total)
