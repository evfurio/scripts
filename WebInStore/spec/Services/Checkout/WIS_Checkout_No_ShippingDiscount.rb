#######################################################################################################################
###This script is used to checkout single or multiple available products with no shipping discount - No PUR applied
######################################################################################################################
###USAGE NOTES
### Run this script using the following command for single or multi cart checkout

###  **Non PUR member, Checkout ONLY REQUIRED PARAMETERS IN THE COMMAND LINE
### d-Con %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\Services\Checkout\WIS_Checkout_No_ShippingDiscount.rb --csv %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\Services\Checkout\wis_checkout_dataset.csv --range "TFS46302"  --or
...
#Add products to cart
        ...
        customer_data.loyalty_customer_id.content = nil
				customer_data.is_guest_checkout.content = true
...
##getting this from the database but haven't set this up for multiple products as of yet
       ...
       ### PURCHASE ###  - before applying the payment. Purchase should not be successful since payment is not applied.
		...
		#There is a open bug where the service response is misspelled as 'Availabile' instead of 'Available' and therefore checking for either one in case it gets corrected
		fulfill_digital_content_rsp.http_body.find_tag("availability_level").content.should == 'Availabile' or 'Available'
		...
		if @complete_checkout == "true"
			...
			calculate_tax_rsp.http_body.find_tag("line_items").at(0).line_item.each do |item|
				...
				total_tax += item.tax.content.to_f
			end
           
		    total_tax  += calculate_tax_rsp.http_body.find_tag("shipping_tax").at(0).content.to_f
			...
			purchase_rsp.http_body.find_tag("total_tax").content.should == round(total_tax)
			
			purchase_rsp.http_body.find_tag("purchase_order").sub_total.content.should == round(expected_subtotal)
			
			#assert actual total is same as expected total
			expected_total = (expected_subtotal + calculated_shipping_cost + total_tax)
			round(purchase_rsp.http_body.find_tag("purchase_order").total.content).should == round(expected_total)
