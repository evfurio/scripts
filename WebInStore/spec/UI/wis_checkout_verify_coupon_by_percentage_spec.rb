# encoding: utf-8
#################################################################################################################################################################################################################################### 
### d-Con %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_checkout_verify_coupon_by_percentage_spec.rb --csv %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_UI_dataset.csv --range TFS49662 --browser ie --or 
### d-Con %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_checkout_verify_coupon_by_percentage_spec.rb --csv %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_UI_dataset.csv --range TFS49661 --browser ie --or 
### d-Con %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_checkout_verify_coupon_by_percentage_spec.rb --csv %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_UI_dataset.csv --range TFS48475 --browser chrome --or 
#################################################################################################################################################################################################################################### 
...
describe "Checkout order level coupon by percentage" do
  ...
  it "#{$tc_id} #{$tc_desc}" do
    ...
    $tracer.trace ("Calculated Cart Sub Total :#{@browser.round(cart_calculated_sub_total)}")
  ...
  def validate_discount_total_order
    sub_total_amount = @browser.subtotal_amount.inner_text.gsub("$", "").to_f
    if (@params["shipping_option"]=="One Day")
      expected_shipping_cost=@current_shipping_cost * @params["percent_off"].to_f / 100
      shipping_cost = @browser.shipping_cost.innerText.gsub("$", "").to_f
      $tracer.trace("Expected Shipping Cost after the promo : #{expected_shipping_cost}")
      $tracer.trace("Actual Shipping Cost after the promo : #{shipping_cost}")
      expected_shipping_cost.should==shipping_cost
    else
      shipping_cost = @browser.shipping_cost.innerText.gsub("$", "").to_f
    end
    ...
    if @browser.sales_tax_amount.length >1
      ...
      sub_total_after_discount=sub_total_amount
    end

    $tracer.trace("Subtotal Amount #{sub_total_amount}")
    $tracer.trace("Subtotal Amount After Discount #{sub_total_after_discount}")
    discount = @browser.discount_amount.inner_text.gsub("$", "")
    discount=discount.gsub("- ", "")

    if (@params["shipping_options"]=="One Day")
      expected_discount="0.00"
    else
      expected_discount=sub_total_amount * (@params["percent_off"].to_f / 100)
    end
    #Validate discount amount.
    $tracer.trace("Expected Discount  : #{@browser.round(expected_discount)}")
    $tracer.trace("Actual Discount    : #{discount}")

    @browser.round(expected_discount).should==discount
    $tracer.trace("Discount Amount : #{discount}")
    calculated_discount= sub_total_amount.to_f - discount.to_f
    $tracer.trace("Calculated Total Amount After Discount : #{@browser.round(calculated_discount)}")
    @browser.round(calculated_discount).should==sub_total_after_discount.to_s

    order_total = @browser.order_total_amount.inner_text.gsub("$", "").to_f
    expected_order_total = (sub_total_amount - discount.to_f + shipping_cost + sales_tax_amount).round(2).to_s
  ...
  def validate_handling_option
    ...
    puts "shiping cost #{@browser.shipping_cost.innerText.gsub("$", "").to_f}"
    @current_shipping_cost=shipping_page_shipping_cost
    if @browser.sales_tax_amount.length >1
