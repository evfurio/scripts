# encoding: utf-8
#################################################################################################################################################################################################################################### 
### d-Con %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_checkout_verify_expired_coupon_spec.rb --csv %QAAUTOMATION_SCRIPTS%\WebInStore\Spec\UI\wis_UI_dataset.csv --range TFS49666 --browser chrome --or 
#################################################################################################################################################################################################################################### 
...
describe "Checkout Verify Expired Coupon" do
  ...
  def validate_discount
    ...
    discount.should=="0.00"
    $tracer.trace("Discount Amount should equal to 0.00")
    ...
    @browser.round(calculated_discount).should==sub_total_amount.to_s
  end
  ...
  def validate_handling_option
    ...
    @browser.validate_label(@browser.trim(@browser.shippingrestriction_message.innerText), @browser.trim(@params["search_value"]))
    # @browser.shippingrestriction_message.innerText.should include "#{@params["search_value"]}"
