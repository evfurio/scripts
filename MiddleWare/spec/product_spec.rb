#d-Con  %QAAUTOMATION_SCRIPTS%\MiddleWare\Spec\C2S\product_spec.rb  --or  --prev_file "C:/tmp/mw/prev_file.product" --curr_file "C:/tmp/mw/curr_file.product"

require "#{ENV['QAAUTOMATION_SCRIPTS']}/MiddleWare/dsl/src/dsl"

describe "MiddleWare Product File Validator" do

  before(:all) do
    # Read previous and current files
    @product_file = ProductFile.new(prev_file_parameter, curr_file_parameter)
  end

  it "should check if previous and current files are identical" do
    @product_file.files_identical?.should be_true
  end

  it "should check if every line of current file is expected length" do
    @product_file.all_records_expected_length?.should be_true
  end

  it "should check for Product numbers in previous file but not in current file" do
    prev_not_current = @product_file.get_keys_in_previous_file_but_not_in_current([:sku])
    if (prev_not_current.any?)
      $tracer.report("Product in previous file but not in current file" + prev_not_current.to_s)
    end
    prev_not_current.none?.should be_true
  end

  it "should check for SKU in current file but not in previous file" do
    current_not_prev = @product_file.get_keys_in_current_file_but_not_in_previous([:sku])
    if (current_not_prev.any?)
      $tracer.report("Product IDs in current file but not in previous file" + current_not_prev.to_s)
    end
    current_not_prev.none?.should be_true
  end

  it "should do regex validation of current file" do
    validation_failures = @product_file.validate_fields
  ...
  it "should check all fields where values in current file differ from that in previous file" do
    differences = @product_file.compare_fields([:sku])
