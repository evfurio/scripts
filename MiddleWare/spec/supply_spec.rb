#d-Con  %QAAUTOMATION_SCRIPTS%\MiddleWare\Spec\C2S\supply_spec.rb  --or  --prev_file "C:/tmp/mw/prev_file.supplysku" --curr_file "C:/tmp/mw/curr_file.supplysku"

require "#{ENV['QAAUTOMATION_SCRIPTS']}/MiddleWare/dsl/src/dsl"

describe "MiddleWare Supply File Validator" do

  before(:all) do
    # Read previous and current files
    @supply_file = SupplyFile.new(prev_file_parameter, curr_file_parameter)
  end

  it "should check if previous and current files are identical" do
    @supply_file.files_identical?.should be_true
  end

  it "should check if every line of current file is expected length" do
    @supply_file.all_records_expected_length?.should be_true
  end

  it "should check for supply in previous file but not in current file" do
    prev_not_current = @supply_file.get_keys_in_previous_file_but_not_in_current([:SKU])
    if (prev_not_current.any?)
      $tracer.report("supply IDs in previous file but not in current file" + prev_not_current.to_s)
    end
    prev_not_current.none?.should be_true
  end

  it "should check for supply in current file but not in previous file" do
    current_not_prev = @supply_file.get_keys_in_current_file_but_not_in_previous([:SKU])
    if (current_not_prev.any?)
      $tracer.report("supply IDs in current file but not in previous file" + current_not_prev.to_s)
    end
    current_not_prev.none?.should be_true
  end

  it "should do regex validation of current file" do
    validation_failures = @supply_file.validate_fields
  ...
  it "should check all fields where values in current file differ from that in previous file" do
    differences = @supply_file.compare_fields([:SKU])
