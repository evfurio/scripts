#USAGE
## from you spec, include the following lines of code.  mod_hosts belongs in the before:each method and mess_cleaner belongs in the after:each method.
...
class ModHost
	...
	def self.mod_hosts(file) file_expected=HOSTS_FILE
		path = "#{ENV['QAAUTOMATION_SCRIPTS']}\\GameStop\\spec\\Configs\\"
		hosts_filepath = "#{path}#{file}"
		hosts_inject(hosts_filepath)
		begin
