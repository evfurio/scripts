SELECT TOP 3 c.availability, c.isavailable, c.condition, d.searchable, D.taxabilitycode, D.variantid AS SKU,D.ProductID AS PRODUCTID, i.skuvariantid, m.[variant.SKU], A.DisplayName AS DISPLAY_NAME, D.cy_list_price AS LIST_PRICE, i.OnHandQuantity, m.[variant.Availability],d.EnableUpsellRecommendations 
...
(SELECT y.ProductID , count(*) 'Variant Count' 
	...
	WHERE c.availability = 'A' and c.isavailable = 1 and c.variantid <> '0' and c.variantid <> '' 
		AND d.searchable =1 AND c.IsDropShip = 0 AND c.condition != 'Digital' and c.condition != 'Download'
		And c.condition =  'Used'
		AND i.onhandquantity > '2' 
		AND d.esrbrating <> 'M'
