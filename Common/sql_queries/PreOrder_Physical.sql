...
WHERE c.isavailable = 1
      AND c.variantid <> '0'
      AND c.variantid <> ''
      AND c.variantid not in ('859996','859998','859997')
      AND d.searchable = 1
      and d.ProductLimit is NULL
      AND c.condition != 'Warranty'
      --AND ((C.Availability = 'A' and i.onhandquantity > '10') OR (C.Availability = 'PR'))
...
from CTE_GameStop where Availability = 'A' and Condition in ('New', 'Used'))-- New
UNION ALL
(select top 1 variantid, esrbrating, condition, Title, Availability, Price, EnableUpsellRecommendations,
ProductID,  taxabilitycode
