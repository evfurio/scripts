SELECT TOP 1
  ...
  C.ProductID,
  D.taxabilitycode,
  C.InternationalShippingRestrictions,
  C.IsDropShip,
  C.InStorePickup
...
WHERE c.availability = 'PR' AND c.variantid <> '0' AND c.variantid <> '' AND c.variantid NOT LIKE '%[a-zA-Z]%'
      AND c.condition != 'Digital' AND c.condition != 'Download' AND c.condition != 'Warranty'
      AND d.searchable = 1 AND d.instorepickup = 1
