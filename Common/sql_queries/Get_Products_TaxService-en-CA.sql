SELECT top 20 D.variantid AS SKU,D.ProductID AS PRODUCTID, 
A.DisplayName AS DISPLAY_NAME, D.cy_list_price AS LIST_PRICE, 
i.OnHandQuantity, D.TaxabilityCode AS TaxCode,d.EnableUpsellRecommendations
FROM [Gamestop_productcatalog].[dbo].[GameStopBase_en-CA_Catalog] A WITH (NOLOCK)
	INNER JOIN 
...
(SELECT y.ProductID , count(*) 'Variant Count' 
	...
	where   (d.EnableUpsellRecommendations = '0' OR d.EnableUpsellRecommendations is NULL)
		ORDER BY NEWID()