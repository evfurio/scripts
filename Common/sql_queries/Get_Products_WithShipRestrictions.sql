SELECT c.availability, c.isavailable, d.searchable, D.variantid AS SKU,D.ProductID AS PRODUCTID, i.skuvariantid, m.[variant.SKU], A.DisplayName AS DISPLAY_NAME, D.cy_list_price AS LIST_PRICE, i.OnHandQuantity, m.[variant.Availability], D.InternationalShippingRestrictions as ShipRestrictions, d.EnableUpsellRecommendations
...
WHERE
c.availability in ('A','PR') and
c.isavailable = 1 and
c.variantid <> '0' and c.variantid <> '' AND
d.searchable = 1 AND
i.onhandquantity > '1000' AND
c.condition != 'Digital' and c.condition != 'Download' AND c.condition != 'Warranty' and
D.ProductLimit is NULL AND
D.InternationalShippingRestrictions is not Null and D.InternationalShippingRestrictions != 'No Restriction' AND
