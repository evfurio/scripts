...
describe "Checkout_GiftCard_GiftOrder" do
  ...
  before(:each) do
	...
	def auto_generate_impulse_username1(t = nil)
      t ||= Time.now
      return "jontestqa+" + t.strftime("%Y%m%d_%H%M%S") + "1@gmail.com"
    end
  ...
  it "should create an account then buy a Gift Order using GameStop Gift Card" do
    
	# set some variables, use defined functions above	
    user_name = auto_generate_impulse_username
	user_name1 = auto_generate_impulse_username1
    password = 'test1234'
    first_name = 'Jon'
    last_name = 'TestQA'
    ...
    # Go back to Impulse hompage (workaround for Gift Order entry)
	@browser.impulse_logo_link.click
    @browser.best_sellers_tab.should_exist
	...
	# enable to purchase as gift
	@browser.purchase_as_gift_button.click
	@browser.email_or_nickname_field.value = user_name1
	@browser.confirm_email_or_nickname_field.value = user_name1
	@browser.message_field.value = "test gift order message"
	@browser.continue_button.click
	
	# Enter GameStop Gift Card Info
