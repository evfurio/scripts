...
describe "Store - Lock Account Suite" do
  before(:all) do
  ...
  after(:all) do
   ...
   it "should verify login and then lockout an account from the Cart Login page " do
	...
	@browser.login_link.click
    @browser.email_address_field.value = @row.find_value_by_name("user")
    @browser.password_field.value = "nachopassword"	
    @browser.sign_in_button.click	
