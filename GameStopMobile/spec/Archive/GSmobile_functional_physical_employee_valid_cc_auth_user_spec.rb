#d-Con %QAAUTOMATION_SCRIPTS%\GameStopMobile\spec\GSmobile_functional_physical_employee_valid_cc_auth_user_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStopMobile\Spec\MobileUI.csv --range "TFS47422" --browser chrome --or 
#
#
#####################################################################################################################################################################
###                                         Product Purchase as Employee Order using Valid CC - Authenticated User                                                ###          
#####################################################################################################################################################################
...
describe "GMobile Functional Tests" do
  before(:all) do
    $tracer.echo = :on
  ...
  it "Product Purchase as Employee Order using Valid CC - Authenticated User" do
    @browser.open(@start_page)
    
	#Empty Shopping Cart/Login Workaround until defect is fixed.
   ...
   # @browser.product_list.at(0).name_link.click  
	...
	#Promo Code Entry.
	@browser.promotion_code_field.value = @promo_code
  ...
  if @browser.chkout_select_existing_address.inner_text.length >= 2
	...
	def initialize_csv_params
	   #Shipping address
		...
		#Employee Promo Code.
		@promo_code = @row.find_value_by_name("DiscountCode")
