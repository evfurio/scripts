#d-Con %QAAUTOMATION_SCRIPTS%\GameStopMobile\spec\GSmobile_functional_giftmsg_under_50_char_auth_user_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStopMobile\Spec\MobileUI.csv --range "TFS47498" --browser chrome --or 
#
#
###########################################################################################################################################################
###                                       Optional Gift Message Field should Accept under 50 - Auth User                                                ###          
###########################################################################################################################################################
...
describe "GSMobile Functional Tests" do
  ...
  it "Optional Gift Message Field should Accept under 50-Auth User" do
    @browser.open(@start_page)
  ...
  if @browser.chkout_select_existing_address.inner_text.length >= 2
	...
	@browser.chkout_gift_message_field.value = "I am testing this box as Auth User"
