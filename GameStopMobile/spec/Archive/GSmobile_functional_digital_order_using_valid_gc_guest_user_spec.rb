#d-Con %QAAUTOMATION_SCRIPTS%\GameStopMobile\spec\GSmobile_functional_digital_order_using_valid_gc_guest_user_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStopMobile\Spec\MobileUI.csv --range "TFS47733" --browser chrome --or 
#
#
###################################################################################################################################################################
###                                        Digital Product Purchase using Valid GC(full payment) - Guest User                                                   ###          
###################################################################################################################################################################
...
describe "GSMobile Functional Tests" do
  ...
  it "Digital Product Purchase using Valid GC(full payment) - Guest User" do
    @browser.open(@start_page)
   ...
   # @browser.product_list.at(0).name_link.click    
	...
	def initialize_csv_params
	   #Billing address.
		...
		@billingEmail = @row.find_value_by_name("BillingEmail")
