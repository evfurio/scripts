#d-Con %QAAUTOMATION_SCRIPTS%\GameStopMobile\spec\GSmobile_functional_physical_order_using_valid_cc_new_user_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStopMobile\Spec\MobileUI.csv --range "TFS47515" --browser chrome --or 
#
#
###########################################################################################################################################################
###                                        Physical Product Purchase using Valid CC - New User                                                          ###          
###########################################################################################################################################################
...
describe "GSMobile Functional Tests" do
  ...
  it "Physical Product Purchase using Valid CC - New User" do
    @browser.open(@start_page)
    ...
    @browser.buy_as_new_account_button.click
    if @browser.timeout(5000).seventeen_or_older_button.exists
      @browser.seventeen_or_older_button.click
    end

	#New Account Login Info
	unique_user = "gs.mobiletester_#{DateTime.now.strftime("%m%d%y%H%M%S")}@gamestop.com"
	@browser.new_login_email_field.value = unique_user
	@browser.new_login_password_field.value = "abc123"
	@browser.new_login_conf_email_field.value= unique_user
	@browser.new_login_conf_password_field.value = "abc123"
