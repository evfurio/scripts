#d-Con %QAAUTOMATION_SCRIPTS%\GameStopMobile\spec\GSmobile_functional_physical_invalid_cc_auth_user_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStopMobile\Spec\MobileUI.csv --range "TFS47427" --browser chrome --or 
#
#
########################################################################################################################################################################
###                                                Physical Product Purchase using InValid CC - Authenticated User                                                   ###          
########################################################################################################################################################################
...
describe "GSMobile Functional Tests" do
  ...
  it "Physical Product Purchase using InValid CC - Auth User" do
    @browser.open(@start_page)
  ...
  if @browser.chkout_select_existing_address.inner_text.length >= 2
   ...
   # Validate Invalid Credit Card Message.
