#d-Con %QAAUTOMATION_SCRIPTS%\GameStopMobile\spec\GSmobile_functional_physical_preorder_using_valid_cc_instore_auth_user_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStopMobile\Spec\MobileUI.csv --range "TFS47425" --browser chrome --or 
#
#
##############################################################################################################################################################
###                                 Preorder Purchase Using Valid CC InStore Pickup - Authenticated User                                                   ###          
##############################################################################################################################################################
...
describe "GSMobile Functional Tests" do
  ...
  it "Preorder Purchase Using Valid CC InStore Pickup - Authenticated User" do
    @browser.open(@start_page)
	...
	if @browser.edit_cart_link.exists
	 @browser.empty_new_cart
	else
	 @browser.shopping_cart_empty_label.should_exist
	end
   ...
   if @browser.chkout_select_existing_billing_address.inner_text.length >= 2
	...
	@browser.chkout_store_pickup_number.value = @ShippingPhone
  ...
  #    @browser.table.id("standard_handling_method_select").find.input.click
   # else
	...
	@browser.instore_pickup_summary_label.should_exist
