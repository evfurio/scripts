#d-Con %QAAUTOMATION_SCRIPTS%\GameStopMobile\spec\GSmobile_functional_physical_and_dlc_order_using_valid_cc_edit_multi_qty_guest_user_spec.rb --csv %QAAUTOMATION_SCRIPTS%\GameStopMobile\Spec\MobileUI.csv --range "TFS47664" --browser chrome --or 
#
#
#####################################################################################################################################################################
###                                            Physical and Digital Order Purchase Using Valid CC Edit Quantity for Both - Guest                                 ###          
#####################################################################################################################################################################
...
describe "GSMobile Functional Tests" do
  ...
  it "Physical and Digital Order Purchase Using Valid CC Edit Quantity for Both-Guest" do
    @browser.open(@start_page)
	...
	@browser.qty_update_field.at(0).value = "2"
	@browser.qty_update_field.at(1).value = "2"
	@browser.cart_edit_done_link.click
	@browser.line_item_label.should_exist
	
	@browser.qty_update_field.at(0).value.should == "2"
	@browser.qty_update_field.at(1).value.should == "2"
